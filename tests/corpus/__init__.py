...
INDUSTRY_CLEAN: dict[str, list[tuple[str, str]]] = {
    ...
    "weight_loss": [
        ("本产品为普通食品，不具有减重功效，请配合合理膳食与规律运动",
         "法定声明：普通食品不宣称功效"),
        ("体重变化因人而异，我们不承诺任何具体的减重数字", "免责声明：不承诺数字"),
        ("体重管理方案请在医生或注册营养师指导下制定", "以专业人士为准"),
        ("我通过控制饮食和规律运动，用三个月减了 8 公斤", "个人经验分享（无产品功效宣称）"),
    ],
    "mother_baby": [
        ...
        ("宝宝发育情况请以儿科医生评估为准，本产品不能替代药物治疗",
         "以医生为准 + 免责声明"),
        ("本品为非特殊医学用途配方食品，不具备疾病预防功能", "资质说明"),
    ],
    "job_recruit": [
        ("本岗位薪资面议，具体以劳动合同约定为准", "薪资披露"),
        ("我们依法为员工缴纳五险一金，入职当日签订劳动合同", "依法用工声明"),
        ("本职位不收取任何费用，请勿向任何个人账户转账", "反诈提示（否定语境）"),
        ("招聘信息已按要求向人社部门备案，收费标准在服务场所公示", "合规声明"),
        # 话题名词 ≠ 违规：政策解读、科普类内容的标题天然会写到"社保挂靠"四个字，
        # 而写这类内容的恰恰是最守规矩的一批人。所以本包只收行动语态
        # （挂靠代缴 / 挂靠社保 / 代缴社保），刻意不收裸「社保挂靠」。
        ("社保挂靠政策解读，本文讲的是养老金计算", "科普语境：话题名词不是违规"),
    ],
    "legal_service": [
        ("我们会在委托前书面告知可能的法律风险，不对办理结果作承诺", "如实告知"),
        ("律师执业证号可在司法部网站核验", "资质核验提示"),
        ("律师代理费按本所公示标准收取，可依法协商", "收费披露"),
        ("案件结果取决于事实、证据和法院裁判，我们只能尽力而为", "免责声明"),
    ],
    "pet": [
        ("本品为宠物配合饲料，不具有疾病治疗功能，请在执业兽医指导下使用",
         "法定声明（含中性术语「饲料」）"),
        ("本产品非兽药，不能替代兽医诊疗", "免责声明"),
        ("换粮请用 7 天过渡法，并观察进食与排便情况", "饲养建议"),
    ],
}
...
INDUSTRY_RISKY: dict[str, list[tuple[str, str]]] = {
    ...
    "mother_baby": [
        ("接近母乳的配方，宝宝更聪明", "接近母乳"),
        ("医院同款，医护级标准，无任何添加", "医院同款"),
        ("宝宝少生病，促进智力发育", "宝宝少生病"),
        ("本品不含激素，媲美母乳", "媲美母乳"),
    ],
    "job_recruit": [
        ("轻松月入三万，无需经验高薪", "轻松月入"),
        ("限男性，年龄限35岁以下，只招本地人", "限男性"),
        ("入职押金 500 元，转正退还", "入职押金"),
        ("专业社保挂靠代缴，随时可办", "挂靠代缴"),
        ("挂靠社保、代缴社保，当月缴纳", "挂靠社保"),
        ("不签合同，试用期不缴社保", "不签合同"),
        ("保证录用，内推保过", "保证录用"),
